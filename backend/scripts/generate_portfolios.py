...
def generate_portfolios(supabase):
    ...
    for risk_level in ["ultra_low", "low", "moderate", "high", "very_high"]:
        ...
        portfolios[risk_level]["prices"] = advisor.prices.iloc[-1]
        ...
        response = supabase.table("Portfolio_Value").select("*", count="exact").eq("portfolio_id", portfolio_id).order("created_at", desc=True).limit(1).execute()
        if not response.data:
            return 
